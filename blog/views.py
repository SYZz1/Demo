...
#主页面
def index(request):
    post_list = getpage(request,Post.objects.all())
    return render(request,'blog/index.html',context={"post_list":post_list})
...
# 归档视图
def archives(request,year,month):
    post_list = getpage(request,Post.objects.filter(created_time__year=year,
                                    created_time__month=month
...
def category(request,pk):
    cate = get_object_or_404(Category,pk=pk)
    post_list = getpage(request,Post.objects.filter(category=cate).order_by('-created_time'))
    return render(request,'blog/index.html',context={'post_list':post_list})

#标签视图
def tag(request,pk):
    cata = get_object_or_404(Tag,pk=pk)
    tag_list = getpage(request,Post.objects.filter(tag=cata))
...
def search(request):
    ...
    if not q :
        ...
        #icontains不区分大小写；contains区分大小写
        post_list = getpage(request,Post.objects.filter(title__icontains=q))
